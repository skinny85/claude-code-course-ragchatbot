...
"""
    ...
    def test_add_course_folder_nonexistent(self):
        ...
        courses_added, total_chunks = self.rag_system.add_course_folder("/nonexistent/path")
    ...
    def test_query_sources_handling(self):
        ...
        self.rag_system.ai_generator.generate_response = Mock(return_value="Test response")
...
class TestRAGSystemWithRealData:
    ...
    def test_load_real_course_documents(self):
        ...
        docs_path = "/Users/adam_ruka/workplace/personal/claude-code-course/lessons-2-to-6/docs"
    ...
    def test_search_functionality_with_real_data(self):
        ...
        if os.path.exists(docs_path):
            ...
            assert not search_results.is_empty(), "Search should find results for 'computer use'"
            
            # Test search tool
            tool_result = self.rag_system.search_tool.execute("computer use")
            print(f"Tool result: {tool_result}")
            
            assert "No relevant content found" not in tool_result, "Tool should find relevant content"
        else:
            pytest.skip("Real docs folder not available")
    ...
    def test_max_results_configuration_impact(self):
        ...
        if os.path.exists(docs_path):
            ...
            assert len(fixed_results.documents) > len(buggy_results.documents), \
